...
<div id="konten">
    <div class="kotak-prediksi box-default">
        ...
        <div class="box-body">
            <div class="callout callout-picture">
                <div class="row">
                    <form method="post" id="jpenumpang_pintu-masuk" action="<?php echo base_url("Prediksi_Penumpang/cari_blok1") ?>">
                        <div class="col-md-4">
                            <select class="form-control select2" id="id_pintu" name="id_pintu" style="width: 100%;" data-placeholder="Pilih Salah Satu" onchange="form.submit()">
                                <?php foreach ($pintu_masuk as $a) { ?>
                                    <option value="<?php echo $a['id']; ?>" <?= $id_pintu == $a['id'] ? "selected" : null  ?>><?php echo $a['pintu_masuk']; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="col-md-2">
    ...
    <div class="panel box box-danger" hidden>
        ...
        <div id="collapseDE" class="panel-collapse collapse in" hidden>
            <div class="box-body">
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($data as $dp) { ?>
                                        <tr>
                                            ...
                                            <?php if ($rank_DES != 0) {
                                                $F_after_DES = round(($alfa * $dp['jumlah']) + ((1 - $alfa) * ($F_before_DES)));
                                            } else {
                                                $F_after_DES = $dp['jumlah'];
                                            } ?>
                            ...
                            <tbody>
                                ...
                                <?php foreach ($data as $dp) { ?>
                                    <tr>
                                        ...
                                        <td><?php echo number_format($dp['jumlah']) ?></td>
                                        <?php if ($rank_DES != 0) {
                                            $F_after_DES = round(($alfa_terkecil_DES * $dp['jumlah']) + ((1 - $alfa_terkecil_DES) * ($F_before_DES)));
                                        } else {
                                            $F_after_DES = $dp['jumlah'];
                                        } ?>
                                        <td><?php echo number_format($F_after_DES) ?></td>
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="panel box box-danger">
                ...
                <div id="collapseThree" class="panel-collapse collapse in">
                    <div class="box-body">
                        <div id="prediksi_penumpang" style="min-width: 310px; height: 600px; margin: 0 auto"></div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-6">
            <div class="panel box box-danger">
                ...
                <div id="collapseThree" class="panel-collapse collapse in">
                    <div class="box-body">
                        <div id="prediksi_penumpang_b" style="min-width: 310px; height: 600px; margin: 0 auto"></div>
                    </div>
                </div>
            </div>
        </div>
...
<table id="data_prediksi_penumpang" border="1" hidden>
    <thead>
        <tr>
            <th>Waktu</th>
            <th>Prediksi Regresi Linear</th>
    ...
    <tbody>
        ...
        <?php foreach ($data as $dp) { ?>
            ...
            <?php $rank_grafik++ ?>
            <?php if ($rank != 0) {
                $F_after = round(($alfa_terkecil_DES * $dp['jumlah']) + ((1 - $alfa_terkecil_DES) * ($F_before)));
            } else {
                $F_after = $dp['jumlah'];
            } ?>
            ...
            <?php
            if ($rank != 0) {
                $F_after2 = round(($alfa_terkecil_DES * $F_after) + ((1 - $alfa_terkecil_DES) * $F_before2));
                $at = round((2 * $F_after) - $F_after2);
                $bt = round(($alfa_terkecil_DES / (1 - $alfa_terkecil_DES)) * ($F_after - $F_after2));
                $DES = round(($at + ($bt)));
            } else {
                $F_after2 = $dp['jumlah'];
                $at = round((2 * $F_after) - $F_after2);
...
<script>
    $(document).ready(function() {
        var table = $('#blok1A').DataTable({
            'buttons': [{
                    extend: 'csvHtml5',
                    title: 'Prediksi Penumpang <?php echo $nama_pintu_masuk->pintu_masuk ?> <?php echo " Tahun " . $tahun; ?> - SIPTA'
                },
                {
                    extend: 'excelHtml5',
                    title: 'Prediksi Penumpang <?php echo $nama_pintu_masuk->pintu_masuk ?> <?php echo " Tahun " . $tahun; ?> - SIPTA'
                ...
                {
                    extend: 'print',
                    title: '<h4>Prediksi Penumpang <?php echo $nama_pintu_masuk->pintu_masuk ?> <?php echo " Tahun " . $tahun; ?> - SIPTA</h4>'
                }
                // {
                //   extend: 'colvis',
...
<script type="text/javascript">
    Highcharts.chart('prediksi_penumpang', {
        data: {
            table: 'data_prediksi_penumpang'
        },
        chart: {
            type: 'line'
        },
        title: {
            useHTML: true,
            text: "<h4>Prediksi Penumpang <?php echo $nama_pintu_masuk->pintu_masuk ?> <?php echo " Tahun " . $tahun; ?></h4>"
        },
        yAxis: {
            allowDecimals: false,
            title: {
...
<script type="text/javascript">
    Highcharts.chart('prediksi_penumpang_b', {
        data: {
            table: 'data_prediksi_penumpang'
        },
        chart: {
